...
while read -r line; do
    raw=$((${#line} + raw))
    temp=$(echo "$line" | sed 's/^./111/') # Starting Double Quotes  plus new quote
    temp=$(echo "$temp" | sed 's/.$/222/') # Ending Double Quotes plus new qoute
    temp=$(echo "$temp" | sed 's/\\x[0-9a-fA-F][0-9a-fA-F]/#####/g') # Hex Codes
...
echo "Literal: $raw"
echo "Parsed: $processed"
echo "2046 is the answer!"
echo "Answer=$((processed - raw))"