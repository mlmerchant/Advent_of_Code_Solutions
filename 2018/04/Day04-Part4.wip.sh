...
while read -r line; do
    if (grep -q "Guard" <<< $line); then
        temp=$(cut -d ' ' -f 4 <<< $line)
        guard[guard_index]=${temp:1}
        ((guard_index++))
    elif (grep -q "sleep" <<< $line); then
        temp=${line:15:2}
        if [[ ${temp:0:1} -eq 0 ]]; then
            temp=${temp:1:1}
        fi
        sleep[sleep_index]=$temp
        ((sleep_index++))
    else
        temp=${line:15:2}
        if [[ ${temp:0:1} -eq 0 ]]; then
...
# Next let's find the guard that sleeps the most.
declare -A total_sleep_time
for individual in ${guard[@]}; do
    total_sleep_time[$individual]=0
done

for i in $(seq 0 $((${#guard[@]} - 1))); do
    individual=${guard[$i]}
    individual_total=${total_sleep_time[$individual]}
    total_sleep_time[$individual]=$((individual_total + (${wake[$i]} - ${sleep[i]}) ))
done

max=$(for num in ${total_sleep_time[@]}; do echo $num; done | sort -n | tail -n 1)
sleepiest_guard=$(for perp in ${!total_sleep_time[@]}; do [[ ${total_sleep_time[$perp]} -eq $max ]] && echo $perp; done)
# echo $sleepiest_guard # is 2857


# Find the sleepest hour for sleepy guard
rm sleep.txt 2> /dev/null
for i in $(seq 0 $((${#guard[@]} - 1))); do
    if [[ ${guard[$i]} -eq $sleepiest_guard ]]; then
        for ((x=${sleep[$i]}; x < ${wake[$i]}; x++)); do
            echo $x >> sleep_hours.txt
        done
    fi
done 
sleepiest_hour=$(sort sleep_hours.txt | uniq -c | sort -nr | head -1)

# echo $sleepiest_hour # is 32

echo $((sleepiest_guard * 32))
